...
CREATE POLICY "Admins can manage daily_check_items" ON public.daily_check_items
  FOR ALL USING (
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_items.tenant_id) AND (profiles.role = 'admin')))
  ) WITH CHECK (
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_items.tenant_id) AND (profiles.role = 'admin')))
  );
...
CREATE POLICY "Drivers and Office can view active daily_check_items" ON public.daily_check_items
  FOR SELECT USING (
    (is_active = TRUE) AND
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_items.tenant_id) AND (profiles.role IN ('driver', 'office', 'admin'))))
  );
...
CREATE POLICY "Admins can manage daily_check_responses" ON public.daily_check_responses
  FOR ALL USING (
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_responses.tenant_id) AND (profiles.role = 'admin')))
  ) WITH CHECK (
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_responses.tenant_id) AND (profiles.role = 'admin')))
  );
...
CREATE POLICY "Drivers can insert their own daily_check_responses" ON public.daily_check_responses
  FOR INSERT WITH CHECK (
    (EXISTS ( SELECT 1 FROM public.profiles WHERE (profiles.user_id = auth.uid()) AND (profiles.tenant_id = daily_check_responses.tenant_id) AND (profiles.role = 'driver') AND (profiles.id = daily_check_responses.driver_id)))
  );
