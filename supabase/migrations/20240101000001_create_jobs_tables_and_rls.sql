...
CREATE INDEX jobs_org_id_created_at_idx ON public.jobs (org_id, created_at DESC);
...
FOR SELECT USING (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = jobs.org_id)
...
FOR INSERT WITH CHECK (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = jobs.org_id AND role IN ('office', 'admin'))
);
...
FOR SELECT USING (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = job_stops.org_id)
);
...
FOR INSERT WITH CHECK (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = job_stops.org_id AND role IN ('office', 'admin'))
);
...
FOR SELECT USING (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = job_events.org_id)
);
...
FOR SELECT USING (
    auth.uid() IN (SELECT id FROM public.profiles WHERE org_id = documents.org_id)
);
